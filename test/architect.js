...
var codeBase={
	x:"var left=(({left-anchor}/100)*rect.width+({left-offset}))+'px';\n"+
		"if(style.left!==left) style.left=left;",
	y:"var right=rect.width-(({right-anchor}/100)*rect.width-({right-offset}))+'px';\n"+
		"if(style.right!==right) style.right=right;",
	x2:"var right=rect.width-(({right-anchor}/100)*rect.width-({right-offset}))+'px';\n"+
		"if(style.right!==right) style.right=right;",
	y2:"var bottom=rect.height-(({bottom-anchor}/100)*rect.height-({bottom-offset}))+'px';\n"+
...
function compileAnchor(elem,layoutVars)
{
	...
	return;
	var codez=[];
	switch(info["left-anchor"]!=0 | (info["left-offset"]!=0)<<1)
	{
		case 0: case 2:
			elem.style.left=info["left-offset"]+'px';
			break;
		case 1:
			elem.style.left=info["left-anchor"]+"%";
			break;
		case 3:
			codez.push(codeBase.x);
	}
	switch(info["top-anchor"]!=0 | (info["top-offset"]!=0)<<1)
	{
		case 0: case 2:
			elem.style.top=info["top-offset"]+'px';
			break;
		case 1:
			elem.style.top=info["top-anchor"]+"%";
			break;
		case 3:
			codez.push(codeBase.y);
	}
	if(info.hasOwnProperty("width-size"))
	{
		// F.log(info["width-size"]);
		elem.style.width=info["width-size"]+"px";
	}
	else
	{
		switch(info["right-anchor"]!=0 | (info["right-offset"]!=0)<<1)
		{
			case 0: case 2:
				elem.style.right=info["right-offset"]+'px';
				break;
			case 1:
				elem.style.right=(100-info["right-anchor"])+"%";
				break;
			case 3:
				codez.push(codeBase.x2);
		}
	}
	if(info.hasOwnProperty("height-size"))
	{
		// F.log(info["height-size"]);
		elem.style.height=info["height-size"]+"px";
	}
	else
	{
		switch(info["bottom-anchor"]!=0 | (info["bottom-offset"]!=0)<<1)
		{
			case 0: case 2:
				elem.style.bottom=info["bottom-offset"]+'px';
				break;
			case 1:
				elem.style.bottom=(100-info["bottom-anchor"])+"%";
				break;
			case 3:
				codez.push(codeBase.y2);
		}
	}
	var codez=codez.join("\n").replace(/\{([\w\-]+)\}/g,function(s,name){
		return info[name];
	});
...
e.setAttribute("data-layout","anchor[left:10 top:10 bottom:90 right:90] offset[top:10 left:10 bottom:10 right:10]");
var lim=1e3;
var tag="wut";
console.time(tag);
for(var x=0;x<lim;++x)
